...
async def statusregular():
    #preip = 'None'
    prestatus = 'Ошибка определения статуса'
    try:
        prestatus = servstatus()
        await bot.send_message(send_id_group, prestatus)  # To group
    except:
        print('Ошибка определения статуса')
    else:
        await bot.send_message(send_id_admin, prestatus)  # to admin
    # try:
    #   preip = getip()
    # except:
    #     print('Ошибка определения своего адреса')
    ...
    while True:
        ...
        # except:
        #     print("Ошибка определения своего адреса")
        #     bot.send_message('Ошибка определения своего адреса')  # to admin
        i += 1
        if i == 1440:
